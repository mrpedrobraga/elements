...
@export_range(1.0, 100.0) var bake_interval = 10.0
@export var color_handle_in := Color.RED:
	set(v): set_color_handle_in(v)
@export var color_handle_out := Color.GREEN:
	set(v): set_color_handle_out(v)
@export var color_path := Color.LIGHT_GRAY:
	set(v): set_color_path(v)
var _curves := {}
...
func get_baked_lengths(start := 0.0, end := 0.0) -> Array:
	...
	for i in curves.size():
		...
		if i + 1 == curves.size():
			...
			end_fraction = snapped(end_fraction, 0.001)
			if end_fraction == 0.0:
				end_fraction = 1.0
...
func _get_array_range(array :Array, start :float, end:float) -> Array:
	...
	end = snapped(end, 0.001) # stepify to elimitate floating-point error
	if end == start:
		end += fsize
