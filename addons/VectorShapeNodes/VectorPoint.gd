...
@export_enum(HandlesType) var handles_type = 4 :
	set(v): set_handles_type(v)
@export_range(0.0, 1.0) var stroke_width := 1.0
@export var stroke_color : Color
@export_range(0.0, 100.0) var bake_interval := 0.0
...
func _get_configuration_warning() -> String:
	...
	if get_handles().size() != expected_handle_count:
		return (
			"Handle type "
			+ str(HandlesType.keys()[handles_type])
			+ " expects "
			+ str(expected_handle_count)
			+ " handles"
		);
	return ""
